    ...
    public function get_buddies() {
        ...
        //CUSTOM_BUDDIES_QUERY_START
            $query = "SELECT DISTINCT f.status_mesg,f.username,f.session_id,f.status,f.guest,f.in_room
                  ...
                  WHERE f.time>" . $this->online_time2 . "
                   AND f.session_id != ".$_SESSION[$this->uid . 'usr_ses_id']."
				   And (f.session_id in (select Friend.UserId
							from Trolo.Friend
							where (Friend.UserId = '".$_SESSION[$this->uid . 'usr_ses_id']."' or Friend.FriendId = '".$_SESSION[$this->uid . 'usr_ses_id']."') and Friend.Status = 1
						)
						Or f.session_id in (Select Friend.FriendId
							from Trolo.Friend
							where (Friend.UserId = '".$_SESSION[$this->uid . 'usr_ses_id']."' or Friend.FriendId = '".$_SESSION[$this->uid . 'usr_ses_id']."') and Friend.Status = 1
						)
						Or f.session_id in (Select `from` from frei_chat
							where (`from` = ".$_SESSION[$this->uid . 'usr_ses_id']." or `to` = ".$_SESSION[$this->uid . 'usr_ses_id'].")
									and `sent` > DATE_ADD('".date("Y-m-d H:i:s")."',INTERVAL -10 MINUTE)
						)
						Or f.session_id in (Select `to` from frei_chat
							where (`from` = ".$_SESSION[$this->uid . 'usr_ses_id']." or `to` = ".$_SESSION[$this->uid . 'usr_ses_id'].")
									and `sent` > DATE_ADD('".date("Y-m-d H:i:s")."',INTERVAL -10 MINUTE)
						)
